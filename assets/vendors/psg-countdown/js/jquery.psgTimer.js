(function () {
    ...
    PsgTimer.prototype.init = function () {
        ...
        timer.container.addClass('psgTimer ' + theme).append(createMarkup(timer));
    ...
    var createMarkup = function (timer) {
        var markup = {};
        var theme = timer.options.theme;
        //labels
        var labels = timer.options.labels;
        markup.labels = '<div class="psgTimer_labels ' + theme + '">';
        for (var unit in labels) {
            if (labels.hasOwnProperty(unit)) {
                markup.labels += '<div class="'+unit+'">' + labels[unit] + '</div>';
            }
        }
        markup.labels += '</div>';
        //numbers
        var countdown = timer.countdown, markup_unit = '';
        for (var unit in countdown) {
            if (labels.hasOwnProperty(unit)) {
                var numberBlocks = '';
                countdown[unit].amount.forEach(function (num) {
                    numberBlocks += numberContainer(timer, num, labels[unit][0]);
                });
                markup_unit += '<div class="' + countdown[unit].className + ' psgTimer_unit ' + theme + '">' + numberBlocks + '</div>';
            }
        }
        markup.numbers = '<div class="psgTimer_numbers ' + theme + '">' + markup_unit + '</div>';
        
        markup.full = markup.numbers + (theme == 'default' ? markup.labels : '');
        //vomit
        return markup.full;
    };

    var numberContainer = function (timer, num, l) {
        var theme = timer.options.theme;
        var markup = '',
            data = 'data-number="' + num + '"';
            l = theme == 'light' ? ' <small>'+l+'</small>' : '';
        var numberBlock = '<div class="number" ' + data + '>' + num + l + '</div>';
            ...
            if (labels.hasOwnProperty(unit)) {
                timer.countdown[unit].amount.forEach(function (number, index) {
                    if (timer.query[unit][index].getAttribute('data-number') !== number) {
                        number = theme == 'light' ? (number + ' <small>'+labels[unit][0]+'</small>') : number;
                        animate_number(timer, timer.query[unit][index], number, timer.options.animation);
                    }
                });
            }
        }
    ...
    var animate_number = function (timer, el, value, animationType) {
        var theme = timer.options.theme;
        var $el = $(el);
        $el.attr('data-number', value);
        if (animationType === 'fade' && theme !== 'light') {
