...
function ajax_data_table(table_id, url, columns, column_defs = [], page_length = 50) {
    //extras
    ...
    //column defs
    /*var columnDefs = [];
    if (column_defs.length) {
        var defs = JSON.parse(column_defs);
        //no searching
        var no_search = defs.no_search || '';
        var no_search_arr = no_search.split(', ');
        console.log(no_search_arr);
        var no_search_obj = [{targets: no_search_arr, searchable: false}];
        columnDefs = [...columnDefs, ...no_search_obj];
        //no ordering
        var no_order = defs.no_order || '';
        var no_order_arr = no_order.split(', ');
        var no_order_obj = [{targets: no_order_arr, orderable: false}];
        columnDefs = [...columnDefs, ...no_order_obj];
        //hidden
        var col_hidden = defs.col_hidden || '';
        var col_hidden_arr = col_hidden.split(', ');
        var col_hidden_obj = [{targets: col_hidden_arr, visible: false}];
        columnDefs = [...columnDefs, ...col_hidden_obj];
    }
    ...
    var table = $('#'+table_id).dataTable({ 
        ...
                });
        },
        destroy: true, //remove previous initialization
        searching: true,
        language: {
           processing: "loading..."
        },
        ...
        lengthChange: true,
        lengthMenu: [ 10, 25, 50, 100, 150, 200, 300, 500, 750, 1000 ],
        // scrollX: true,
        stateSave: false,
        processing: true,
        serverSide: true,
...
function client_datatable(table_id, obj = {}) {
    //Non-server side datatables
    var table = $('#'+table_id).DataTable({ 
        "paging": true,
        "pageLength" : obj.page_length ? obj.page_length : 25,
        "lengthChange": obj.length_change ? obj.length_change : true, 
        "searching": obj.searching ? obj.searching : true,
        "info": obj.info ? obj.info : true,
        "scrollX": true,
        "autoWidth": false,
        "ordering": obj.ordering ? obj.ordering : true,
        "stateSave": true,
        "processing": obj.processing ? obj.processing : false, 
        "pagingType": "simple_numbers", 
        "language": {
            "search": "Search/filter: ",
            "emptyTable": obj.empty_table ? obj.empty_table : 'Nothing to show.',
        },
        "columnDefs": obj.column_defs ? obj.column_defs : [],
    });
}