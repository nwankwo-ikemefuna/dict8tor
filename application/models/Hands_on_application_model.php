...
class Hands_on_application_model extends Core_Model {
	...
	public function sql($to_join = [], $select = "*", $where = []) {
		...
		$select .= join_select($arr, 'full_name', full_name_select('hop', false));
		...
		$select .= join_select($arr, 'grant_project_lga_name', "p_lga.name");
		$select .= join_select($arr, 'grant_project_budget_amount', price_select2($this->local_currency_code, 'hop.grant_project_budget'));
		$select .= join_select($arr, 'applicant_status_text', case_map_select('hop.application_status', ACC_STATUSES));
		$select .= join_select($arr, 'ngo_registered_text', case_map_select('hop.ngo_registered', ['No', 'Yes']));
		...
		//project lgas
		if (in_array('p_lga', $to_join) || in_array('all', $to_join)) {
			$joins = array_merge($joins, 
				[T_LGAS.' p_lga' => ['p_lga.id = hop.grant_project_lga']]
			);
		} 
	...
	public function form_elements_applicant($states = [], $lgas = []) {
		$form_elements = [
			...
			['name' => 'state', 'title' => 'State of Origin', 'type' => 'select', 'class' => '', 'options' => $states, 'text_col' => 'name', 'id' => 'nigerian_states', 'value' => CRS_ID, 'required' => true, 'col' => 6, 'render' => 'state_name'],
			['name' => 'lga', 'title' => 'LGA of Origin', 'type' => 'select', 'class' => '', 'options' => $lgas ?? [], 'text_col' => 'name', 'required' => true, 'col' => 6, 'render' => 'lga_name'],
	...
	public function form_elements_grant($lgas = []) {
		$form_elements = [
			...
			['name' => 'grant_project_lga', 'title' => 'Project Location (LGA)', 'type' => 'select', 'class' => '', 'options' => $lgas ?? [], 'text_col' => 'name', 'required' => true, 'col' => 4, 'render' => 'grant_project_lga_name'],
			...
			['name' => 'grant_project_budget', 'title' => 'Proposed budget', 'type' => 'number', 'class' => '', 'required' => true, 'col' => 12, 'render' => 'grant_project_budget_amount'],
	...
	public function form_elements_ngo() {
		$form_elements = [
			...
			['name' => 'ngo_registered', 'title' => 'Is organisation registered with Corporate Affairs Commission?', 'type' => 'select', 'class' => '', 'id' => 'ngo_registered', 'options' => [0 => 'No', 1 => 'Yes'], 'required' => false, 'col' => 12, 'assoc' => true, 'render' => 'ngo_registered_text'],
