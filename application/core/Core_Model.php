...
class Core_Model extends CI_Model {
    ...
    private function table_alias($table) {
        ...
        $_delim = preg_match('/(^|\s)AS($|\s|[^w])/', $table) ? 'AS' : ' ';
     ...
     * @return object
     */
    public function get_row($table, $id = '', $by = '', $trashed = 0, $joins = [], $select = '', $where = [], $group_by = '', $data_type = 'object') {
        $alias = $this->table_alias($table);
        if (strlen($id)) {
            $by = strlen($by) ? $by : 'id';
            $where[$alias.'.'.$by] = $id;
        ...
        $data_type = $data_type == 'object' ? 'row' : 'row_array';
        return $this->db->get()->$data_type();
    }
     ...
     * @return object
     */
    public function get_rows($table, $trashed = 0, $joins = [], $select = '', $where = [], $order = null, $group_by = '', $limit = '', $offset = 0, $having = '', $data_type = 'object') {
        $this->prepare_query($table, $trashed, $joins, $select, $where, $order, $group_by, $limit, $offset, $having);
        $data_type = $data_type == 'object' ? 'result' : 'result_array';
        return $this->db->get()->$data_type();
    }
     ...
     * @return object
     */
    public function get_rows_ajax($table, $keys, $buttons, $trashed = 0, $joins = [], $select = '', $where = [], $order = null, $group_by = '', $limit = '', $having = '', $offset = 0) {
    ...
    public function next_order($table, $where = []) {
        //ensure order column exists
        if ( ! $this->db->field_exists('order', $table)) return; 
        $count = $this->count_rows($table, $where);
        if ($count > 0) { 
            return $this->get_aggr_row($table, 'max', 'order', $where) + 1;
        } 
        return 1;
    }


    public function map_row($rows, $by) {
        $data = [];
        foreach ($rows as $row) {
            $map_key = $row->$by ?? $row->id; //default to id if map field is not set
            $data[$map_key] = $row;
        }
        return $data;
    }
    ...
    public function map_rows($rows, $by) {
        $data = [];
        foreach ($rows as $row) {
            $map_key = $row->$by ?? $row->id; //default to id if map field is not set
            $data[$map_key][] = $row;
    ...
    public function exists($table, $where, $exclude_id = null) {
        //excluding any (current record during edit)
        if (strlen($exclude_id)) {
            $where = array_merge($where, ['id != ' => $exclude_id]);
        }
        $count = $this->count_rows($table, $where);
        return ($count > 0);
    }
